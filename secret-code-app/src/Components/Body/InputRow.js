...
const Input = (props) => {
  ...
  const checkHandler = (event) => {
    ...
    for (const item in number) {
      ...
      if (
        originalData[item] === number[item] && number[item] !== ''
      ) {
        circle.push("blue");
        originalData[item] = "";
        number[item] = "";
    ...
    if (
      ...
      data.enableIndex < props.length - 1
		...
		props.dispatch({type:'updateEnableIndex', enableIndex: data.enableIndex + 1});
    } else {
      if (data.enableIndex === 7 && (circle.includes("white") || circle.length !== 4 )) {
        result = "YOU LOST";
      } else {
